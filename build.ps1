...
$dist       = Join-Path $root 'dist\v1.3.0'
...
if ($Arch -eq 'all') {
    ...
    $stage = Join-Path $root 'release-staging\v1.3.0\diagnostic'
    ...
    $zipOut = Join-Path $root 'release-staging\v1.3.0\SurfaceChargingTrayDiagnostic.zip'
