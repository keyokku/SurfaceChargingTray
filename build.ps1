...
$dist       = Join-Path $root 'dist\v1.4.3'
...
if ($Arch -eq 'all') {
    ...
    $stage = Join-Path $root 'release-staging\v1.4.3\diagnostic'
    ...
    $zipOut = Join-Path $root 'release-staging\v1.4.3\SurfaceChargingTrayDiagnostic.zip'
