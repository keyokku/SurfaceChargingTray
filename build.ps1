...
$dist       = Join-Path $root 'dist\v1.4.1'
...
if ($Arch -eq 'all') {
    ...
    $stage = Join-Path $root 'release-staging\v1.4.1\diagnostic'
    ...
    $zipOut = Join-Path $root 'release-staging\v1.4.1\SurfaceChargingTrayDiagnostic.zip'
