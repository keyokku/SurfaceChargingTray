...
<p align="center">
  ...
  <img width="650" alt="image" src="https://github.com/user-attachments/assets/492ee973-d329-49ce-812b-1d85bd2f232b" />
</p>
...
|---|---|---|
| <sub>[v1.3.0-beta1](https://github.com/keyokku/SurfaceChargingTray/releases/tag/v1.3.0-beta1) *(beta — older Surface UI test)*</sub> | <sub>2026-05-14</sub> | <sub>Auto-detects whether your Surface app exposes the three-radio classic UI or the single-button *Charge to 100%* override. Reshapes the tray menu, settings, hotkeys, and scheduler around what your device actually supports. **Beta — for users whose Surface app shows only the one-button toggle; regular users on the three-radio UI should stay on v1.2.2.**</sub> |
| **[v1.2.2](https://github.com/keyokku/SurfaceChargingTray/releases/tag/v1.2.2)** *(latest stable)* | 2026-05-12 | Charging-mode scheduler. Press a hotkey before bed; the device enters simulated sleep to change charging mode, then lets the device sleep normally. See [Charging-mode scheduler](#charging-mode-scheduler) below. **Adds a separate diagnostic tool download for users whose Surface model isn't detected, plus better error dialog that links. No AHK package — `.exe` builds only.** |
